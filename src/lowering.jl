...
function lower(builder::Builder, mir::MIR, eq)
    eq = value(eq)
...
function lower_terminal(builder::Builder, mir::MIR, eq)
    if is_number(eq)
        val = Float64(eq)
...
function lower_uniop(builder::Builder, mir::MIR, eq)
    _, op, x = arguments(eq)
    s = lower(builder, mir, x)
    return push_new_reg!(mir, r -> uniop(r, op, s))
end

function lower_binop(builder::Builder, mir::MIR, eq)
...
function lower_ternary(builder::Builder, mir::MIR, eq)
...
function lower_bincall(builder::Builder, mir::MIR, eq)
    op, x, y = arguments(eq)
